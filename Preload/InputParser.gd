...
var MOVEMENTS = [
	...
	{"name": "RAPID_GRAB", "pattern": [Combo.GRAB, Combo.GRAB, Combo.GRAB]},
	{"name": "RAPID_THROW", "pattern": [Combo.THROW, Combo.THROW, Combo.THROW]},
	{"name": "RAPID_KUZUSHI", "pattern": [Combo.KUZUSHI,Combo.KUZUSHI,Combo.KUZUSHI]},
	...
	{"name": "GRAB_COUNTER", "pattern": [Combo.GRAB_COUNTER]},
	{"name": "KUZUSHI_COUNTER", "pattern": [Combo.KUZUSHI_COUNTER]},
	{"name": "THROW_COUNTER", "pattern": [Combo.THROW_COUNTER]},
...
func process_direction_buffer():
	...
	if matched_move != "":
		...
		# Commenting out current_move_sequence.clear() makes it so that motion inputs no longer work
		# Quater circles don't output special moves anymore 
		#current_move_sequence.clear()
		# Commenting out direction_history.clear() makes the motion inputs work, but they repeat multiple times for some reason. 
		# Maybe I clear direction_history and move
...
func get_current_actions() -> int:
	...
	if Input.is_action_just_pressed("GRAB"):
		...
		return Combo.THROW
	elif Input.is_action_just_pressed("COUNTER_GRAB"):
		return Combo.GRAB_COUNTER
	elif Input.is_action_just_pressed("COUNTER_KUZUSHI"):
		return Combo.KUZUSHI_COUNTER
	elif Input.is_action_just_pressed("COUNTER_THROW"):
